const gutsSword = {
  weapon: "Guts Greatsword",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/c/c4/ER_Icon_weapon_Greatsword.png",
  armour: "Crucible Axe/Bullgoat Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Bull-Goat's Talisman",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Ritual Sword Talisman",
  physick1: "Stonebarb Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 50,
  mind: 26,
  endurance: 40,
  strength: 71,
  dexterity: 12,
  intelligence: 10,
  faith: 10,
  arcane: 10,
  comments: "Use Lion's Claw ash of war with heavy affinity",
};

const giantCrusher = {
  weapon: "Giant Crusher",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/9/9b/ER_Icon_weapon_Giant-Crusher.png",
  armour: "Crucible Axe/Bullgoat Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Bull-Goat's Talisman",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Ritual Sword Talisman",
  physick1: "Stonebarb Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 50,
  mind: 26,
  endurance: 40,
  strength: 71,
  dexterity: 12,
  intelligence: 10,
  faith: 10,
  arcane: 10,
  comments: "Use Lion's Claw ash of war with heavy affinity",
};

const maraisExecutionersSword = {
  weapon: "Marais Executioner's Sword",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/0/03/ER_Icon_weapon_Marais_Executioner%27s_Sword.png",
  armour: "Briar Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Godfrey Icon",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Rotten Winged Sword Insignia",
  physick1: "Magic-Shrouding Cracked Tear",
  physick2: "Thorny Cracked Tear",
  vigor: 50,
  mind: 25,
  endurance: 25,
  strength: 55,
  dexterity: 14,
  intelligence: 10,
  faith: 10,
  arcane: 40,
  comments:
    "Swap Godfrey Icon with Bull-Goat's Talisman for extra poise during weapon skill",
};

const darkmoonGreatsword = {
  weapon: "Dark Moon Greatsword",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/8/8c/ER_Icon_weapon_Dark_Moon_Greatsword.png",
  armour: "Spellblade Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Godfrey Icon",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Magic Scorpion Charm",
  physick1: "Magic-Shrouding Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 40,
  mind: 25,
  endurance: 32,
  strength: 16,
  dexterity: 11,
  intelligence: 70,
  faith: 25,
  arcane: 10,
  comments:
    "OP BUILD. 25 Faith to use flame grant me strength and golden vow. Pair with Carian Regal Scepter for Ranni's Dark Moon and other sorceries",
};

const deathsPoker = {
  weapon: "Deaths Poker",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/a/a6/ER_Icon_weapon_Death%27s_Poker.png",
  armour: "Spellblade Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Graven-Mass Talisman",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Magic Scorpion Charm",
  physick1: "Magic-Shrouding Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 40,
  mind: 25,
  endurance: 32,
  strength: 16,
  dexterity: 11,
  intelligence: 70,
  faith: 25,
  arcane: 10,
  comments:
    "OP BUILD. 25 Faith to use flame grant me strength and golden vow. Pair with Carian Regal Scepter for Ranni's Dark Moon and other sorceries",
};

const moonveil = {
  weapon: "Moonveil Katana",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/9/91/ER_Icon_weapon_Moonveil.png",
  armour: "Ronin Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Carian Filigreed Crest",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Magic Scorpion Charm",
  physick1: "Magic-Shrouding Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 45,
  mind: 22,
  endurance: 25,
  strength: 16,
  dexterity: 45,
  intelligence: 56,
  faith: 10,
  arcane: 10,
  comments:
    "Split Dexterity and Intelligence stats for dealing damage with Transient Moonlight wave, and up close",
};

const handOfMalenia = {
  weapon: "Hand of Malenia",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/2/2a/ER_Icon_weapon_Hand_of_Malenia.png",
  armour: "Malenia's Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Bull-Goat's Talisman",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Rotten Winged Sword Insignia",
  physick1: "Thorny Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 50,
  mind: 23,
  endurance: 25,
  strength: 16,
  dexterity: 70,
  intelligence: 10,
  faith: 25,
  arcane: 10,
  comments: "25 Faith to use flame grant me strength and golden vow",
};

const uchigatana = {
  weapon: "Uchigatana",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/b/b3/ER_Icon_weapon_Uchigatana.png/",
  armour: "Land of Reeds Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Lord of Blood's Exultation",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Green Turtle Talisman",
  physick1: "Dexterity-knot Crystal Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 55,
  mind: 15,
  endurance: 35,
  strength: 24,
  dexterity: 70,
  intelligence: 10,
  faith: 10,
  arcane: 10,
  comments:
    "Pair with Seppuku ash of war with bleed affinity for extra bleed damage. Can swap out Land of Reeds Headgear with White Mask for extra bleed damage",
};

const boltOfGransax = {
  weapon: "Bolt of Gransax",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/9/90/ER_Icon_weapon_Bolt_of_Gransax.png",
  armour: "Tree Sentinel Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Godfrey Icon",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Lightning Scorpion Charm",
  physick1: "Lightning-Shrouding Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 35,
  mind: 30,
  endurance: 29,
  strength: 20,
  dexterity: 75,
  intelligence: 10,
  faith: 20,
  arcane: 10,
  comments:
    "OP BUILD. 20 faith is because with Godrick's greatrune you can use golden vow and flame grant me strength",
};

const blasphemousBlade = {
  weapon: "Blasphemous Blade",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/8/87/ER_Icon_weapon_Blasphemous_Blade.png",
  armour: "Briar Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Fire Scorpion Charm",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Ritual Sword Talisman",
  physick1: "Flame-Shrouding Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 40,
  mind: 23,
  endurance: 26,
  strength: 30,
  dexterity: 20,
  intelligence: 10,
  faith: 70,
  arcane: 10,
  comments:
    "OP BUILD. High Faith can use flame grant me strength and golden vow/howl of shabriri",
};

const sacredRelicSword = {
  weapon: "Sacred Relic Sword",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/e/e1/ER_Icon_weapon_Sacred_Relic_Sword.png",
  armour: "Blackflame Monk Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Bull-Goat's Talisman",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Sacred Scorpion Charm",
  physick1: "Flame-Shrouding Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 50,
  mind: 23,
  endurance: 30,
  strength: 14,
  dexterity: 40,
  intelligence: 10,
  faith: 52,
  arcane: 10,
  comments:
    "High Faith can use flame grant me strength and golden vow/howl of shabriri",
};

const godslayersGreatsword = {
  weapon: "Godslayer's Greatsword",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/0/0a/ER_Icon_weapon_Godslayer%27s_Greatsword.png",
  armour: "Godskin Noble Hood/Leda's Armour",
  talisman1: "Shard of Alexander",
  talisman2: "Bull-Goat's Talisman",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Fire Scorpion Charm",
  physick1: "Flame-Shrouding Cracked Tear",
  physick2: "Opaline Hardtear",
  vigor: 50,
  mind: 25,
  endurance: 32,
  strength: 20,
  dexterity: 40,
  intelligence: 10,
  faith: 42,
  arcane: 10,
  comments:
    "High Faith can use flame grant me strength and golden vow/howl of shabriri",
};

const riversOfBlood = {
  weapon: "Rivers of Blood",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/a/a9/ER_Icon_weapon_Rivers_of_Blood.png",
  armour: "White Mask/Black Knife Armour Set",
  talisman1: "Shard of Alexander",
  talisman2: "Rotten Winged Sword Insignia",
  talisman3: "Lord of Blood's Exultation",
  talisman4: "Carian Filigreed Crest",
  physick1: "Flame-Shrouding Cracked Tear",
  physick2: "Thorny Cracked Tear",
  vigor: 50,
  mind: 30,
  endurance: 18,
  strength: 12,
  dexterity: 46,
  intelligence: 10,
  faith: 15,
  arcane: 48,
  comments: "15 Faith to use flame grant me strength",
};

const dragonCommunion = {
  weapon: "Dragon Communion Seal",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/b/bf/ER_Icon_weapon_Dragon_Communion_Seal.png",
  armour: "Drake Knight Armour Set",
  talisman1: "Roar Medalion",
  talisman2: "Flock's Canvas Talisman",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Primal Glintstone Blade",
  physick1: "Flame-Shrouding Cracked Tear",
  physick2: "Magic-Shrouding Cracked Tear",
  vigor: 50,
  mind: 30,
  endurance: 29,
  strength: 11,
  dexterity: 11,
  intelligence: 10,
  faith: 25,
  arcane: 69,
  comments:
    "Pair with Ripple Blade for S scaling with arcane. 25 Faith to use flame grant me strength and golden vow",
};

const mohgwynsSacredSpear = {
  weapon: "Mohgwyn's Sacred Spear",
  weaponImg:
    "https://static.wikia.nocookie.net/eldenring/images/1/18/ER_Icon_weapon_Mohgwyn%27s_Sacred_Spear.png",
  armour: "White Mask/Lord of Blood's Set",
  talisman1: "Shard of Alexander",
  talisman2: "Lord of Blood's Exultation",
  talisman3: "Dragoncrest Greatshield Talisman",
  talisman4: "Fire Scorpion Charm",
  physick1: "Flame-Shrouding Cracked Tear",
  physick2: "Greenburst Crystal Tear",
  vigor: 50,
  mind: 23,
  endurance: 22,
  strength: 25,
  dexterity: 14,
  intelligence: 10,
  faith: 25,
  arcane: 60,
  comments:
    "OP BUILD. 25 Faith to use flame grant me strength and golden vow, can pair with dragon communion seal because of high arcane",
};

let buildsArray = [
  gutsSword,
  giantCrusher,
  maraisExecutionersSword,
  darkmoonGreatsword,
  deathsPoker,
  moonveil,
  handOfMalenia,
  uchigatana,
  boltOfGransax,
  blasphemousBlade,
  sacredRelicSword,
  godslayersGreatsword,
  riversOfBlood,
  dragonCommunion,
  mohgwynsSacredSpear,
];

export default function buildStructs(stat: string | undefined) {
  let validBuilds;
  switch (stat) {
    case "int": {
      validBuilds = [darkmoonGreatsword, deathsPoker, moonveil];
      break;
    }
    case "str": {
      validBuilds = [gutsSword, giantCrusher, maraisExecutionersSword];
      break;
    }
    case "dex": {
      validBuilds = [handOfMalenia, uchigatana, boltOfGransax];
      break;
    }
    case "fai": {
      validBuilds = [blasphemousBlade, sacredRelicSword, godslayersGreatsword];
      break;
    }
    case "arc": {
      validBuilds = [riversOfBlood, dragonCommunion, mohgwynsSacredSpear];
      break;
    }
    case "rand": {
      buildsArray.sort(() => Math.random() - 0.5);
      validBuilds = [buildsArray[0], buildsArray[1], buildsArray[2]];
      break;
    }
    default: {
      validBuilds = undefined;
      break;
    }
  }
  return validBuilds;
}
